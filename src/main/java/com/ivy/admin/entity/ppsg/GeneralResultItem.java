package com.ivy.admin.entity.ppsg;


public class GeneralResultItem {

    private String generalName;

    private Integer force;//总武力
    private Integer intellect;//总智力
    private Integer troops;//总兵力
    private Integer combat;//总战力

    private Integer f;//基础满级武力
    private Integer i;//基础满级智力
    private Integer t;//基础满级兵力
    private Integer c;//基础满级战力

    private Integer f5;//科技武力
    private Integer i5;//科技智力
    private Integer t5;//科技兵力
    private Integer c5;//科技战力

    private Integer f6;//四圣石武力
    private Integer i6;//四圣石智力
    private Integer t6;//四圣石兵力
    private Integer c6;//四圣石战力

    private Integer f7;//战器武力
    private Integer i7;//战器智力
    private Integer t7;//战器兵力
    private Integer c7;//战器战力

    private Integer f9;//兵种兵书武力
    private Integer i9;//兵种兵书智力
    private Integer t9;//兵种兵书兵力
    private Integer c9;//兵种兵书战力

    private Integer f15;//将魂武力
    private Integer i15;//将魂智力
    private Integer t15;//将魂兵力
    private Integer c15;//将魂战力

    private Integer f17;//战阵武力
    private Integer i17;//战阵智力
    private Integer t17;//战阵兵力
    private Integer c17;//战阵战力

    private Integer f18;//命格武力
    private Integer i18;//命格智力
    private Integer t18;//命格兵力
    private Integer c18;//命格战力

    private Integer f19;//幻化武力
    private Integer i19;//幻化智力
    private Integer t19;//幻化兵力
    private Integer c19;//幻化战力

    private Integer f20;//阵法武力
    private Integer i20;//阵法智力
    private Integer t20;//阵法兵力
    private Integer c20;//阵法战力

    public GeneralResultItem() {
    }

    public String getGeneralName() {
        return generalName;
    }

    public void setGeneralName(String generalName) {
        this.generalName = generalName;
    }

    public Integer getF9() {
        return f9;
    }

    public void setF9(Integer f9) {
        this.f9 = f9;
    }

    public Integer getI9() {
        return i9;
    }

    public void setI9(Integer i9) {
        this.i9 = i9;
    }

    public Integer getT9() {
        return t9;
    }

    public void setT9(Integer t9) {
        this.t9 = t9;
    }

    public Integer getC9() {
        return c9;
    }

    public void setC9(Integer c9) {
        this.c9 = c9;
    }

    public Integer getF() {
        return f;
    }

    public void setF(Integer f) {
        this.f = f;
    }

    public Integer getI() {
        return i;
    }

    public void setI(Integer i) {
        this.i = i;
    }

    public Integer getT() {
        return t;
    }

    public void setT(Integer t) {
        this.t = t;
    }

    public Integer getF5() {
        return f5;
    }

    public void setF5(Integer f5) {
        this.f5 = f5;
    }

    public Integer getI5() {
        return i5;
    }

    public void setI5(Integer i5) {
        this.i5 = i5;
    }

    public Integer getT5() {
        return t5;
    }

    public void setT5(Integer t5) {
        this.t5 = t5;
    }

    public Integer getF6() {
        return f6;
    }

    public void setF6(Integer f6) {
        this.f6 = f6;
    }

    public Integer getI6() {
        return i6;
    }

    public void setI6(Integer i6) {
        this.i6 = i6;
    }

    public Integer getT6() {
        return t6;
    }

    public void setT6(Integer t6) {
        this.t6 = t6;
    }

    public Integer getF15() {
        return f15;
    }

    public void setF15(Integer f15) {
        this.f15 = f15;
    }

    public Integer getI15() {
        return i15;
    }

    public void setI15(Integer i15) {
        this.i15 = i15;
    }

    public Integer getT15() {
        return t15;
    }

    public void setT15(Integer t15) {
        this.t15 = t15;
    }

    public Integer getF17() {
        return f17;
    }

    public void setF17(Integer f17) {
        this.f17 = f17;
    }

    public Integer getI17() {
        return i17;
    }

    public void setI17(Integer i17) {
        this.i17 = i17;
    }

    public Integer getT17() {
        return t17;
    }

    public void setT17(Integer t17) {
        this.t17 = t17;
    }

    public Integer getF18() {
        return f18;
    }

    public void setF18(Integer f18) {
        this.f18 = f18;
    }

    public Integer getI18() {
        return i18;
    }

    public void setI18(Integer i18) {
        this.i18 = i18;
    }

    public Integer getT18() {
        return t18;
    }

    public void setT18(Integer t18) {
        this.t18 = t18;
    }

    public Integer getF19() {
        return f19;
    }

    public void setF19(Integer f19) {
        this.f19 = f19;
    }

    public Integer getI19() {
        return i19;
    }

    public void setI19(Integer i19) {
        this.i19 = i19;
    }

    public Integer getT19() {
        return t19;
    }

    public void setT19(Integer t19) {
        this.t19 = t19;
    }

    public Integer getF20() {
        return f20;
    }

    public void setF20(Integer f20) {
        this.f20 = f20;
    }

    public Integer getI20() {
        return i20;
    }

    public void setI20(Integer i20) {
        this.i20 = i20;
    }

    public Integer getT20() {
        return t20;
    }

    public void setT20(Integer t20) {
        this.t20 = t20;
    }

    public Integer getForce() {
        return force;
    }

    public void setForce(Integer force) {
        this.force = force;
    }

    public Integer getIntellect() {
        return intellect;
    }

    public void setIntellect(Integer intellect) {
        this.intellect = intellect;
    }

    public Integer getTroops() {
        return troops;
    }

    public void setTroops(Integer troops) {
        this.troops = troops;
    }

    public Integer getCombat() {
        return combat;
    }

    public void setCombat(Integer combat) {
        this.combat = combat;
    }

    public Integer getC() {
        return c;
    }

    public void setC(Integer c) {
        this.c = c;
    }

    public Integer getC5() {
        return c5;
    }

    public void setC5(Integer c5) {
        this.c5 = c5;
    }

    public Integer getC6() {
        return c6;
    }

    public void setC6(Integer c6) {
        this.c6 = c6;
    }

    public Integer getC15() {
        return c15;
    }

    public void setC15(Integer c15) {
        this.c15 = c15;
    }

    public Integer getC17() {
        return c17;
    }

    public void setC17(Integer c17) {
        this.c17 = c17;
    }

    public Integer getC18() {
        return c18;
    }

    public void setC18(Integer c18) {
        this.c18 = c18;
    }

    public Integer getC19() {
        return c19;
    }

    public void setC19(Integer c19) {
        this.c19 = c19;
    }

    public Integer getC20() {
        return c20;
    }

    public void setC20(Integer c20) {
        this.c20 = c20;
    }

    public Integer getF7() {
        return f7;
    }

    public void setF7(Integer f7) {
        this.f7 = f7;
    }

    public Integer getI7() {
        return i7;
    }

    public void setI7(Integer i7) {
        this.i7 = i7;
    }

    public Integer getT7() {
        return t7;
    }

    public void setT7(Integer t7) {
        this.t7 = t7;
    }

    public Integer getC7() {
        return c7;
    }

    public void setC7(Integer c7) {
        this.c7 = c7;
    }
}
